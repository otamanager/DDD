		...
		<div class="row">
			<div class="col-sm-3 col-md-2 sidebar">
				<ul class="nav nav-sidebar">
					<li class="active"><a href="getNoProcessOrders">未处理订单 <span
			...
			<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
				<h1 class="page-header">未处理现场入(出)库单</h1>
							...
							<c:forEach items="${sceneOrders }" var="order">
								<c:if test="${order.isPass==0 }">
									<tr>
										...
										<td><c:if test="${order.sceneOrderType==1 }">现场入库单</c:if>
											<c:if test="${order.sceneOrderType==2 }">现场出库单</c:if></td>
										<td><a
											href="displayScenerOrderAndOrder?code=<c:out value="${order.code }"/>&orderCode=<c:out value="${order.orderCode }"/>"
